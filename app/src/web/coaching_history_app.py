...
# Page configuration
st.set_page_config(
    page_title="Coaching Feedback Generator", page_icon="🚚", layout="wide"
)
...
def generate_coaching_feedback(query: str, session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        request_data = {"query": query, "session_id": session_id}

        # Make API request
        response = requests.post(f"{API_URL}/coaching-feedback", json=request_data)
...
def main():
    ...
    if not st.session_state.conversation_started:
        st.info(
            "👋 Hello! I'm your coaching assistant. I can help you generate coaching feedback for employees based on their history. You can ask me about specific employees, severity categories, or enter a general coaching query."
        )
            ...
            for i in range(0, len(examples), 2):
                if st.button(f"{examples[i]}", key=f"example_{i}"):
                    ...
                    result = generate_coaching_feedback(
                        examples[i], st.session_state.session_id
                    )
