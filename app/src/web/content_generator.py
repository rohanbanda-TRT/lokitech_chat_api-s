    ...
    try:
        ...
        if content_type_changed and previous_content_type:
            if message:
                ...
                modified_message = (
                    f"[Content type changed from {previous_content_type} to {subject}]"
                )
...
def main():
        ...
        if not st.session_state.user_info_submitted:
            with st.form("user_info_form"):
                ...
                subject = st.selectbox(
                    "Content Type", CONTENT_TYPES, index=0
                )  # Default to SMS (first option)
            ...
            **Current Session**
            ...
            new_content_type = st.selectbox(
                ...
                CONTENT_TYPES,
                index=(
                    CONTENT_TYPES.index(st.session_state.subject)
                    if st.session_state.subject in CONTENT_TYPES
                    else 0
                ),
            )
            ...
            if new_content_type != st.session_state.subject:
                ...
                st.session_state.messages.append(
                    {
                        "role": "system",
                        "content": f"Content type changed to: **{new_content_type}**",
                    }
                )
    ...
    if st.session_state.user_info_submitted:
        ...
        st.info(
            "Please fill out your information in the sidebar to start generating content."
        )
