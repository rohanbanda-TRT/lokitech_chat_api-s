...
class ApplicantDetails(BaseModel):
    ...
    mobileNumber: str
    applicantStatus: str
    applicantID: Optional[int] = None  # Add applicantID field, make it optional for backward compatibility
...
class DSPApiClient:
    ...
    def get_applicant_details(self, dsp_code: str, station_code: str = "DJE1", applicant_id: int = 5) -> Optional[ApplicantDetails]:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Failed to retrieve applicant details. Status code: {response.status_code}")
    ...
    def update_applicant_status(self, dsp_code: str, applicant_id: Optional[int] = None, 
                               current_status: str = "SENT", new_status: str = "INPROGRESS", 
                               applicant_data: Dict[str, Any] = None) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"Updating applicant status for DSP code: {dsp_code} from {current_status} to {new_status} with emp_id: {emp_id}")
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Failed to update applicant status. Status code: {response.status_code}")
                logger.error(f"Response: {response.text}")
                return False
